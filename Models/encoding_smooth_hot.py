...
def dynamic_smooth_encoding_torch(values,
    ...
    return encoded
def one_hot_encoding_torch(values, min_val=-15, max_val=15, num_bins=30):
    """
    对三维张量 (B, L, 1) 或 (B, L) 进行 one-hot 编码。
    输出形状: (B, L, num_bins)
    """
    # 如果输入是 (B, L, 1)，去掉最后一维
    if values.dim() == 3 and values.size(-1) == 1:
    ...
    B, L = values.shape
    values = torch.clamp(values, min=min_val, max=max_val - 1e-8)  # 防止溢出
    ...
    # 初始化 one-hot 编码
    encoded = torch.zeros(B, L, num_bins, device=values.device, dtype=values.dtype)
    encoded.scatter_(2, bin_idx.unsqueeze(-1), 1.0)  # 主 bin 权重设为1
